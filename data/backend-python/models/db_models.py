"""
Modelos SQLAlchemy (ORM) generados a partir de db/init.sql.

Schemas: processing (jobs, stats, configs), auth (users, workspaces, projects, RBAC).
Usar para migración incremental desde consultas directas a ORM.
"""
...
def gen_uuid() -> Any:
    """Generador de UUID para default en Python (alternativa a uuid_generate_v4() en BD)."""
    return uuid4()
...
class Module(Base):
    """auth.modules - Módulos funcionales (logs, projects, workspaces, etc.)."""
...
class Role(Base):
    """auth.roles - Roles del sistema (super_admin, workspace_admin, project_admin, analyst, viewer)."""
    __tablename__ = "roles"
    __table_args__ = {"schema": "auth"}
